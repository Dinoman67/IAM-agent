...
## Information architecture (landing + 5-section shell)
...
- **App shell** (`App.tsx` + `components/layout/Rail.tsx`) — fixed left rail
  (`Run · Evidence · Policy · Exports · Audit`), icon-only under `md:`,
...
- **`SimulationPage` (Run)** — pickers, FLOW + LIVE LOG twin boxes with timed
  ...
  interpreter, verdicts gated behind kernel entry, scenario explainer cards.
